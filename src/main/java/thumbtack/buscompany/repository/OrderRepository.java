...
public interface OrderRepository {
    ...
    @Results(id = "order", value = {
            @Result(property = "tripDay", column = "tripDayId", javaType = TripDay.class,
                    one = @One(select = "thumbtack.buscompany.repository.TripDayRepository.getTripDay", fetchType = FetchType.LAZY)),
    ...
    @Select("SELECT ")
    Order getByTipDayId(@Param("tripDayId") Integer tripDayId);
